...
\begin{center}
    ...
    {\large
    \textbf{Pragnyan Ramtha}\textsuperscript{1}
    }\\[0.5cm]

    {\small
    \texttt{pragnyanramtha@gmail.com}
    }\\[1.5cm]
...
The rapid scaling of Large Language Models (LLMs) from billions to hundreds of billions of parameters has driven remarkable improvements in language understanding and generation tasks~\cite{brown2020language, touvron2023llama2}. However, the architectural foundation of modern LLMs,the Transformer with its quadratic-complexity self-attention mechanism~\cite{vaswani2017attention},imposes fundamental constraints on context window sizes.
...
\begin{techniquebox}{ALiBi (Attention with Linear Biases)~\cite{press2022train}}
...
\textbf{Performance}: 11\% faster training, successful 2K$\rightarrow$10K+ extrapolation with only 11\% perplexity degradation.
...
\begin{techniquebox}{RoPE (Rotary Position Embedding)~\cite{su2021roformer}}
...
\begin{techniquebox}{Position Interpolation (PI)~\cite{chen2023extending}}
...
\begin{techniquebox}{LongRoPE~\cite{ding2024longrope}}
...
\begin{techniquebox}{StreamingLLM: Attention Sinks~\cite{xiao2023streaming}}
...
\begin{techniquebox}{Gated Attention~\cite{sun2024gatedattention}}
...
\begin{techniquebox}{Mamba~\cite{gu2024mamba}}
...
\begin{techniquebox}{Mamba-2~\cite{dao2024mamba2}}
...
\begin{techniquebox}{RWKV~\cite{peng2023rwkv}}
...
\begin{techniquebox}{KV Cache Quantization~\cite{hooper2024kvquant}}
...
\begin{techniquebox}{H2O: Heavy-Hitter Oracle~\cite{zhang2024h2o}}
...
\textbf{Combination}: Quantization + pruning stack multiplicatively ($3\times \times 2.5\times \approx 8\times$).
\end{techniquebox}
...
\begin{techniquebox}{LLMLingua~\cite{jiang2023llmlingua}}
...
\begin{techniquebox}{LongLLMLingua~\cite{jiang2024longllmlingua}}
...
\begin{techniquebox}{ICAE (In-Context Autoencoder)~\cite{ge2024icae}}
...
\begin{techniquebox}{Gisting~\cite{mu2024gisting}}
...
\textbf{Failures Identified}: ``Lost by boundary'' (information at chunk boundaries), ``Lost if surprise'' (unexpected content), ``Lost along the way'' (gradual degradation).
\end{techniquebox}
...
\begin{techniquebox}{Retrieval-Augmented Generation~\cite{lewis2020retrieval}}
\textbf{Pipeline}:
\begin{enumerate}
...
\begin{warningbox}
\textbf{Universal Phenomenon}: All tested models show U-shaped retrieval accuracy~\cite{liu2024lost}:
\begin{center}
\begin{tabular}{lccc}
\toprule
...
\begin{techniquebox}{Graph of Records~\cite{zhang2024gor}}
...
\begin{techniquebox}{BriefContext~\cite{zhang2025briefcontext}}
...
\begin{techniquebox}{Claude Prompt Caching~\cite{anthropic2024caching}}
...
\begin{techniquebox}{FlashAttention-3~\cite{shah2024flashattention3}}
\textbf{Evolution}:
\begin{itemize}
...
\begin{techniquebox}{PagedAttention (vLLM)~\cite{kwon2023vllm}}
...
\textbf{Position Interpolation} & \textbf{5.4} $\checkmark$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Context rot} is fundamental: $\text{accuracy\_drop}(\%) \approx 0.5 \times \ln(\text{context\_KB})$
...
\newblock \emph{Advances in Neural Information Processing Systems}, 30:5998--6008, 2017.
...
\bibitem[Ding et~al.(2024)]{ding2024longrope}
...
Bo Peng, Eric Alcaide, Quentin Anthony, Alon Albalak, et~al.
\newblock RWKV: Reinventing RNNs for the transformer era.
\newblock \emph{arXiv preprint arXiv:2305.13048}, 2023.

\bibitem[Sun et~al.(2024)]{sun2024gatedattention}
Yutao Sun, Li Dong, Shaohan Huang, Shuming Ma, et~al.
\newblock Gated attention: A unified framework for eliminating attention sinks.
\newblock \emph{Advances in Neural Information Processing Systems}, 37, 2024.

\bibitem[Zhang et~al.(2024a)]{zhang2024h2o}
Zhenyu Zhang, Ying Sheng, Tianyi Zhou, Tianlong Chen, et~al.
...
\newblock In \emph{Proceedings of EMNLP 2023}, pages 13358--13376, 2023.

\bibitem[Jiang et~al.(2024)]{jiang2024longllmlingua}
Huiqiang Jiang, Qianhui Wu, Xufang Luo, Dongsheng Li, et~al.
...
\bibitem[Ge et~al.(2024)]{ge2024icae}
...
\bibitem[Mu et~al.(2024)]{mu2024gisting}
...
\bibitem[Zhang et~al.(2024b)]{zhang2024gor}
Haozhen Zhang, Tao Xu, Wentao Wang, Yu Bai, et~al.
\newblock Graph of records: Boosting retrieval augmented generation for long-context summarization with graphs.
\newblock \emph{arXiv preprint arXiv:2410.11001}, 2024.

\bibitem[Zhang et~al.(2025)]{zhang2025briefcontext}
Chen Zhang, Yang Liu, Jiaying Zhou, Zhiyu Wang, and Jing Jiang.
\newblock BriefContext: Efficient long-context reasoning via partitioning.
\newblock \emph{arXiv preprint arXiv:2401.00789}, 2025.

\bibitem[Liu et~al.(2024)]{liu2024lost}
Nelson~F Liu, Kevin Lin, John Hewitt, Ashwin Paranjape, et~al.
...
\newblock \emph{Proceedings of the ACM SIGOPS 29th Symposium on Operating Systems Principles}, pages 611--626, 2023.
...
\bibitem[Shah et~al.(2024)]{shah2024flashattention3}
...
Cheng-Ping Hsieh, Simeng Sun, Samuel Krez, et~al.
...
\bibitem[Kamradt(2023)]{kamradt2023needle}
Greg Kamradt.
\newblock Needle in a haystack - pressure testing LLMs.
\newblock \emph{GitHub repository}, 2023.

\bibitem[Anthropic(2024)]{anthropic2024caching}
Anthropic.
\newblock Prompt caching for Claude.
\newblock Technical report, Anthropic, 2024.

\bibitem[OpenAI(2024)]{openai2024caching}
OpenAI.
\newblock OpenAI prompt caching.
